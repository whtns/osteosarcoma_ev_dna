...
steps_to_process_all = ["fastqmcf", "trimmomatic", "prinseq", "fastqc", "hisat2"]
...
parser.add_argument("-r", "--reference", dest="reference_genome", default= "./ucsc.hg19.fasta", help="reference genome")
...
fastq_location = sys.argv[1]
reference_genome = sys.argv[2]
steps_to_process = sys.argv[3].split(",")
#genome_file = sys.argv[4]
bowtie_index = "/media/thor/storage/Homo_sapiens/UCSC/hg19/Sequence/BowtieIndex/genome"
...
fastq_clumped = clumpify_output_dir+fastq_base_filename+".c2.fastq.gz"
max_subs = "2"
...
bbmerge_output_dir = output_directory + "bbmerge/"
bbmerge_fastq = bbmerge_output_dir+ fastq_base_filename + "_bbmerge.fastq"
unmerged_fastq = bbmerge_output_dir+ fastq_base_filename + "_unmerged.fastq"
bbmerge_hist = bbmerge_output_dir+ fastq_base_filename + "_hist.txt"
if(not os.path.isfile(bbmerge_fastq)) and(("bbmerge" in steps_to_process) or ("all" in steps_to_process)):
	subprocess.call(["mkdir",bbmerge_output_dir])
	print "running bbmerge"
	cmd = BBMerge[:]
	#print cmd
	cmd.append("in="+prinseq_good_fastq)
	cmd.append("out="+bbmerge_fastq)
	cmd.append("outu="+unmerged_fastq)
	cmd.append("ihist="+bbmerge_hist)
	errcode = subprocess.call(cmd)
	if(errcode == 0):
		print "bbmerge for sample "+fastq_base_filename+" finished successfully"
	else:
		print "bbmerge for sample"+fastq_base_filename+" failed !!!!"
		del steps_to_process[:]
	del cmd
...
remove_duplicates_bam= picard_output_dir+ fastq_base_filename + "_removed_duplicates.bam"
remove_dup_metrics = picard_output_dir+fastq_base_filename+"_removed_dup_metrics.txt"
if(("removeduplicates" in steps_to_process) or ("all" in steps_to_process)):
	...
	out_file = picard_output_dir + fastq_base_filename+"_removeduplicates.log"
	err_file = picard_output_dir + fastq_base_filename+"_removeduplicates.err"
	print " ".join(cmd)
...
bam_index= picard_output_dir+fastq_base_filename+"_sorted.bam.bai"
...
# FEATURECOUNTS
# =====================================================================
featurecounts_output = picard_output_dir + fastq_base_filename+ "_featurecounts/"
gtf_location = "/home/thor/storage/Homo_sapiens/UCSC/hg19/Annotation/Genes/genes.gtf"
if("featurecounts" in steps_to_process) or ("all" in steps_to_process):
	cmd = featurecounts[:]
	cmd.append("-r")
	...
	cmd.append("-s")
	cmd.append(fastq_base_filename+"|"+sortsam_bam+"|"+"notes")
	cmd.append("-t")
